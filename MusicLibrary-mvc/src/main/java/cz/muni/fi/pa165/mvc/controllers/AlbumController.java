...
public class AlbumController {
	...
	@RequestMapping(value = "/{id}/upload-cover", method = RequestMethod.POST, consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
	@PreAuthorize("isAuthenticated()")
	public String handleFileUpload(@PathVariable Long id, @RequestParam("file") MultipartFile file, RedirectAttributes redirectAttributes) {
		...
		try {
			...
			Long lid = artFacade.createArt(artDTO);
			artDTO = artFacade.getArtById(lid);

			albumFacade.updateArt(id, artDTO);
